...
- The `$$` in both `$$A8C_CI_TOOLKIT_PLUGIN_DIR` and `$$PATH`. A single `$` is resolved when the pipeline is uploaded; `$$` defers it to when the job runs. Escape whichever variables only get their intended value at runtime: `$A8C_CI_TOOLKIT_PLUGIN_DIR` is exported by this plugin's `environment` hook, and `$$PATH` refers to the container's `$PATH` (a single `$PATH` would capture the uploader's PATH), so neither should be expanded at upload time. `$RELEASE_VERSION` above keeps a single `$` for the opposite reason — it is set on the build, so it already has a value when the pipeline is uploaded. To avoid weighing this up line by line, have `command:` call a dedicated `.sh` script instead: nothing inside a `.sh` file is interpolated at upload time, so a plain `export PATH=$PATH:/a8c-ci-toolkit/bin` works there.
